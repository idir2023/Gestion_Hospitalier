...
if (isset($_POST['modifier'])) {
    ...
    $stmt = $pdo->prepare("UPDATE utilisateur SET nom=?, prenom=?, email=? ,mot_de_passe=?,photo=? WHERE id=?");
    $stmt->execute([$nom, $prenom, $email, $mot_de_passe, $photo, $id]);

    if ($stmt->rowCount() > 0) {
        echo '<script> alert("Les données ont été mises à jour avec succès dans la base de données."); window.location="admin.php"; </script>';
    } else {
