...
<body class="hold-transition sidebar-mini">
    ...
    <div class="container-fluid mt-3">
      ...
      <div class="row">
        ...
        <?php
                    ...
                    $stmt = $pdo->prepare("SELECT * FROM utilisateur,doctor WHERE utilisateur.id=doctor.id_user  AND utilisateur.id = :id");
                    $stmt->bindParam(":id", $id_p);
                    $stmt->execute();
                    $result = $stmt->fetch(PDO::FETCH_ASSOC); ?>
          <h2 class="text text-center bg-secondary">Bienvenue Dr.<?php echo $result['nom_d'] .' '. $result['prenom_d'] ; ?></h2>
          <hr>
        </div>
      </div>
      <!-- Cards -->
  <div class="table-responsive">
    ...
    try {
        $stmt = $pdo->prepare("SELECT * FROM utilisateur,doctor WHERE utilisateur.id=doctor.id_user 
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-9">
                            <h5 class="card-subtitle mb-2 text-muted">Informations personnelles</h5><hr>
                            <p class="card-text"><strong>Nom:</strong> <?php echo $result['nom_d'] ?></p>
                            <p class="card-text"><strong>Prénom:</strong> <?php echo $result['prenom_d'] ?></p>
                            ...
                            <a href="crud1.php?update=<?php echo $result['id_user']; ?>" class="btn btn-primary">Modifier</a>
                        </div>
                    </div>
                </div>
            </div>
